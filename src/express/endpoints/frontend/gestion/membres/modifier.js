...
module.exports = {
    ...
    "execute": async function (routerRequest, routerResponse) {
        ...
        const requiredPermissions = ["ADMIN", "MANAGE_MEMBERS"];
        ...
        // On obtient l'ID de l'utilisateur à modifier et on récupère ses informations
        const id = routerRequest.params.id;

        const utilisateurInfo = await databaseManager.utilisateurs.getUtilisateur({ id });
        if(!utilisateurInfo) return routerResponse.redirect("/admin/membres");

        // On affiche la page de login
        routerResponse.status(200).render("admin/membres/editMembre", { user: routerRequest.user, utilisateurInfo });
